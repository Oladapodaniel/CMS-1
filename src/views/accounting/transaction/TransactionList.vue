    ...
    <div class="main-con">
      <div class="main-body">
        <div class="top container-wide mt-3">
          ...
          <div class="actions">
            <button class="more-btn align-items-center default-btn border-0" @click="toggleTransac">Add Income<span><i class="fa fa-angle-down btn-icon"></i></span>
            </button>

            <button class="more-btn align-items-center default-btn border-0" @click="toggleTransac">Add Expense<span><i class="fa fa-angle-down btn-icon"></i></span>
            </button>
         ...
         <div class="container-wide">
           <div class="col-12 col-sm-6 col-lg-4 px-0 mt-5">
            <div class="d-flex">
              <button class="form-control transaction-button close-modal" @click="toggleAccount">
            ...
            <div class="style-category close-modal" v-if="accountDisplay">
                      ...
                      <div class="header-border close-modal">
                        <div @click="transactionItem" class="manual-dd-item close-modal" v-for="(cash, index) in cashAndBank" :key="index">
                            <div class="d-flex justify-content-between py-1 px-3 close-modal">
                                <div class="close-modal">{{ cash.name.type }}</div> 
                                <div class="close-modal">{{ cash.name.amount }}</div>     
                            </div>                     
                        </div>
                      </div>
                      <div class="desc-head py-1 px-3 close-modal">Credit Card</div>
                      <div class="header-border close-modal">
                        <div @click="transactionItem" class="manual-dd-item close-modal" v-for="(credit, index) in creditCard" :key="index">
                            <div class="d-flex justify-content-between py-1 px-3 close-modal">
                    ...
                    <!-- <div class="row"> -->
                      <div class="col-md-12 create-event px-2 d-flex justify-content-between close-modal">
                        <a href="#" class="craete-event-btn font-weight-bold p-3 close-modal"><i class="fa fa-file-text-o close-modal" aria-hidden="true"></i>  Upload Bank Statement</a>
                        <a
                          class="craete-event-btn font-weight-bold px-2 py-3 close-modal"
                          @click="openModal"
                          ><i class="fa fa-plus-circle" aria-hidden="true"></i>  Add a new account</a
                        >
                      </div>
                    <!-- </div> -->
        ...
        <!-- <h5>Modal</h5> -->
<Dialog header="Add an Account" :visible="displayModal" :style="{width: '50vw'}" :modal="true">
    <div class="row mt-2">
                ...
                <div class="desc-head py-1 px-3 close-modal">Assets</div>
                      <div class="header-border close-modal">
                        <div v-if="filterAccount.length > 0">
                            <div @click="transactionItem" class="manual-dd-item close-modal" v-for="(item, index) in filterAccount" :key="index">
                      ...
                      <div class="header-border close-modal">
                        <div v-if="filterLiabilities.length > 0">
                            <div @click="transactionItem" class="manual-dd-item close-modal" v-for="(item, index) in filterLiabilities" :key="index">
                            <div class="d-flex justify-content-between py-2 px-3 close-modal">
                                <div class="close-modal offset-sm-1">{{ item }}</div> 
                ...
                />
                      ...
                      <div class="header-border close-modal" v-if="filterCurrency.length > 0">
                        <div class="manual-dd-item close-modal" v-for="item in filterCurrency" :key="item.id">
                            <div class="d-flex justify-content-between py-2 px-3 close-modal">
                                <div class="close-modal offset-sm-1">{{ item.name }} - {{ item.country }}</div>      
                            </div>                      
                        </div>
                      </div>
...
<div class="parent-table">
      ...
      <div class=" col-12 parent-desc p-2 pl-4" v-for="(item, index) in transactions" :key="index">
          <div><input type="checkbox" v-model="item.check" class="form-check"></div>
          <div class="desc">{{ item.date }}</div>
          <div>
              <div class="desc-head">{{ item.descHead }}</div>
              <div class="desc">{{ item.desc }}</div>
          </div>
          <div class="desc-head">{{ item.amount }}</div>
          <div class="choose-cat">{{ item.category }}</div>
          <div>{{ item.mark }}</div>
      </div>
        </div>
...
import TransactionForm from "./EditTransaction"
export default {
    components: {
    ...
    setup () {
        const transactions = ref([
            { check: false, date: 'feb 17, 2021', descHead: 'Write a descrition', desc: 'Cash on Hand', amount: 'N0.00', category: 'Choose a category', mark: 'Marked' },
            { check: false, date: 'feb 17, 2021', descHead: 'Write a descrition', desc: 'Cash on Hand', amount: 'N0.00', category: 'Choose a category', mark: 'Marked' },
            { check: false, date: 'feb 17, 2021', descHead: 'Write a descrition', desc: 'Cash on Hand', amount: 'N0.00', category: 'Choose a category', mark: 'Marked' },
            { check: false, date: 'feb 17, 2021', descHead: 'Write a descrition', desc: 'Cash on Hand', amount: 'N0.00', category: 'Choose a category', mark: 'Marked' },
            { check: false, date: 'feb 17, 2021', descHead: 'Write a descrition', desc: 'Cash on Hand', amount: 'N0.00', category: 'Choose a category', mark: 'Marked' },
        ...
        const transactionItem = (e) => {
            ...
            console.log(e.target.children[0].innerHTML, e.target.children[0].nextElementSibling.innerHTML)
    ...
    const toggleTransac = (e) => {
      console.log(e.target.innerText == "Add Income")
    ...
    const transacObj = (payload) => {
      transactions.value.push(payload)
        ...
        return {
            transactions, filterFormIsVissible, toggleFilterFormVissibility, toggleSearch, searchIsVisible, cashAndBank, creditCard, accountDisplay, toggleAccount, hideModals, selectedTransaction,  transactionItem, displayModal, openModal, closeModal, showAccount, accountType, liabilities, getCurrenciesFromCountries, currencyList, showCurrency, selectAccount, filterCurrency, currencyText, accountText, filterAccount, filterLiabilities, showEditTransaction, toggleTransac, closeIt, transacObj, transacPropsValue
        }
    }
}
</script>
...
.more-btn:first-child, .more-btn:nth-child(2) {
    margin-right: 20px
}
