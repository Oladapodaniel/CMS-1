...
<div @click="hideModals">
    ...
    <div class=" col-12 parent-desc d-flex justify-content-between first p-3">
        ...
        <i class="pi pi-times" @click="closeTransac" aria-hidden="true"></i>
      </div>
      <div class="container">
          <div class="row mt-4">
              <div class="col-12">
              ...
              <div class="col-7 pr-0">
                  ...
                  <input class="form-control" id="description" ref="descrp" v-model="transacObj.descHead" placeholder="Description" :autofocus="showEditTransaction">
              </div>
              <div class="col-5 pl-0">
                  <div class="label-text">Date</div>
                  <input type="date" v-model="transacObj.date" id="date" class="form-control">
              </div>
          </div>
          <div class="row mt-3">
              <div class="col-7 pr-0">
                  ...
                  <div
                    class="select-elem-con pointer form-control d-flex justify-content-space-between align-items-center close-modal"
                    @click="showAccount = !showAccount"
                    >
                    <span class="ofering close-modal">{{ !transacObj.accountFlow ? "Select": transacObj.accountFlow }}</span
                    ><span>
                        <i
            ...
            <div
              ...
              <div class="col-sm-5 pl-0">
                  ...
                  <input type="number" class="form-control" v-model="transacObj.amount" placeholder="0.00">
              </div>
              <!-- <div class="col-12 text-right vendor mt-1">{{ transacProp.vendor }}</div> -->
          </div>
          <!-- <div class="row mt-3"> -->
              <!-- <div class="col-6 pr-0">
                  <Dropdown v-model="transacObj.transacType" class="w-100" :options="[{name: 'Withdrawal'},{name: 'Deposit'}]" optionLabel="name" placeholder="Select" />
              </div> -->
              <!-- <div class="col-sm-6">
          ...
          <div class="row mt-3">
            <div class="col-12">
                <div class="label-text">{{ transacProp.account }}</div>
                <div
                    class="select-elem-con pointer d-flex justify-content-space-between close-modal"
                    @click="showUncategorized = !showUncategorized"
                    >
                    <span class="ofering close-modal">{{ !transacObj.category ? "Select" : transacObj.category }}</span
                    ><span>
                        <i
                        class="pi pi-angle-down close-modal"
                        aria-hidden="true"
                ...
                <div class="desc-head py-1 px-3 close-modal">Assets</div>
                      <div class="header-border close-modal">
                        <div v-if="filterUncategorizedAsset.length > 0">
                            <div class="manual-dd-item close-modal" v-for="(item, index) in filterUncategorizedAsset" :key="index">
                            <div class="d-flex justify-content-between py-2 px-3 close-modal">
                                <div class="close-modal offset-sm-1" @click="categories">{{ item }}</div>      
                            </div>                      
                        </div>
                        </div>
                        <div v-else>
                      ...
                      <div class="header-border close-modal">
                        <div v-if="filterUncategorizedLiabilities.length > 0">
                            <div class="manual-dd-item close-modal" v-for="(item, index) in filterUncategorizedLiabilities" :key="index">
                            <div class="d-flex justify-content-between py-2 px-3 close-modal">
                                <div class="close-modal offset-sm-1" @click="categories">{{ item }}</div> 
                ...
                <div class="d-flex justify-content-end">
                    <div class="mt-2 vendor">{{ transacProp.type }}</div>
...
<!-- Split category select -->
                <div v-else>
                 ...
                 <div class="row mt-3" v-for="(item, index) in transacObj.splitCategories" :key="index">
                  <!-- <div class="col-sm-7" style="border: 2px solid green">{{ item.category }}</div> -->
                  <!-- <div class="col-sm-5" style="border: 2px solid green">{{ item.amount }}</div> -->
                  <div class="col-sm-7 pr-0">
                    ...
                    >
                    <span class="ofering close-modal">{{ item.category ? item.category : "Select" }}</span
                    ><span>
                        <i
                        class="pi pi-angle-down close-modal"
                        aria-hidden="true"
                ...
                <div class="desc-head py-1 px-3 close-modal">Assets</div>
                      <div class="header-border close-modal">
                        <div v-if="filterUncategorizedAsset.length > 0">
                            <div class="manual-dd-item close-modal" v-for="(item, index) in filterUncategorizedAsset" :key="index">
                            <div class="d-flex justify-content-between py-2 px-3 close-modal">
                                <div class="close-modal offset-sm-1" @click="categoryAccount">{{ item }}</div>      
                            </div>                      
                        </div>
                        </div>
                        <div v-else>
                  ...
                  <div class="col-sm-4 pl-0">
                        <div class="label-text">Amount</div>
                        <input type="number" class="form-control" ref="amountRef" v-model="item.amount" placeholder="0.00">
                    </div>
                    <div class="col-sm-1 pl-0">
                        <div class="label-text"></div>
                        <i class="pi pi-trash mt-4" @click="deleteSplit(index)"></i>
                    </div>
              ...
              <!-- <div class="d-flex justify-content-end">
                    <div class="mt-2 vendor">{{ transacProp.customer }}</div>
                    <div class="dot ml-2"></div>
                    <div class="mt-2 ml-2 vendor">Include Tax Sales</div>
                </div> -->
            </div>
            ...
            <div class="row mt-2">
                ...
                <div class="col-6 mt-2 text-center split" v-tooltip.top="'Create multiple categories to associate(split) this trasaction between different accounts.'" @click="splitWithdrawal">Split this widrawal  <i class="fa fa-info-circle"></i>
                </div>
                ...
                <div class="error-div col-10 offset-1 mt-3" v-if="parseInt(totalAmount.amount) > transacObj.amount">
                    ...
                    <p class="error-message col-10 pl-0">The sum of the above lines should not exceed the total deposit amount of {{ transacObj.amount }}</p>
                    </div>
                </div>
                <div class="col-12 mt-4">
                    <textarea class="form-control" v-model="transacObj.note" rows="3" placeholder="Notes"></textarea>
                </div>
                <div  class="col-12 mt-1 modified">Transaction last modified on February 18th,2021</div>
...
export default {
    ...
    props: ['transacProp',  'showEditTransaction'],
    setup (props, { emit } ) {
        ...
        const filterUncategorizedAsset = computed(() => {
            if (uncategorizedText.value !== "" && accountType.value.length > 0) {
                return accountType.value.filter((i) => {
                if (i)  return i.toLowerCase().includes(uncategorizedText.value.toLowerCase())
                })
                // console.log(currencyText)
        ...
        const categoryAccount = (e) => {
            ...
            transacObj.value.splitCategories[transacObj.value.splitCategories.length - 1].category = e.target.innerHTML
            transacObj.value.splitCategories[transacObj.value.splitCategories.length - 1].showUncategorized = false
        ...
        return {
            showAccount, accountText, filterAccount, accountType, filterLiabilities, liabilities, hideModals,showUncategorized, uncategorizedText, filterUncategorizedAsset, filterUncategorizedLiabilities, closeTransac, transacObj, saveTransac, categoryAccount, splitWithdrawal, deleteSplit, accountFlow, totalAmount, amountRef, descrp, categories
        }
    }
}
</script>
