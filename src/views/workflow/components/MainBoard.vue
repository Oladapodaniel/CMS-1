                            ...
                            <div class="col-8 border animate" :class="{ 'col-md-8': showTriggers || done, 'col-md-11': !showTriggers &&  workflow.triggers.length > 0 && !done }">
                                <div class="row" :class="{ 'd-none': workflow.triggers.length === 0 || done }">
                                    <div class="col-12 animate border  scr-height"  style="height: 400px" :class="{ 'col-md-4': actionSelected, 'col-md-6': !actionSelected, 'd-none': done }">
                                        ...
                                        <GroupAddOrRemove :groups="groups" v-else-if="selectedTrigger.triggerType === 10" />
                                            ...
                                            <div class="border scr-height" :class="{ 'col-md-6 ani': actionSelected, 'd-none': !actionSelected || !selectedAction || !(selectedActionIndex >= 0) || triggerActions.length < 1 }">
                                                <div class="row h-100" style="overflow-y:scroll">
                                                    <div class="col-md-12" >
                                                        ...
                                                        <WhatsAppAction 
                                                            v-else-if="selectedAction && selectedAction.actionType === 13"
                                                            :selectedActionIndex="selectedActionIndex"
                                                            @updateaction="updateAction"
                                                        />
                                                        <AddNoteAction 
...
export default {
    ...
    setup () {
        ...
        const getWorkflowById = async id => {
            try {
                const { returnObject: data } = await workflow_service.getById(id);
                const res = await workflow_service.getById(id);
                console.log(res, "rs");
                ...
                // workflow.value.triggers = data.triggers.map(i => {
                //     const t = triggers.find(j => j.triggerType === i.triggerType);
                //     return {
                //         ...i,
                //         icon: t.icon ? t.icon : '',
                ...
                console.log(workflow, "workflow by id");
            } catch (error) {
                console.log(error);
            }
        }
